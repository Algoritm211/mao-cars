...
import {
  COMMENT_FILTER
} from "@/system/car-details/components/auction-comments-section/components/comment-form/models/form-constants";
...
export const CommentForm = () => {
  ...
  const onMobileFilterChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const newFilterObj = COMMENT_FILTER
      .find(elem => elem.key === event.target.value)!
  ...
  return (
    <div className='flex flex-col gap-2'>
      <div className='flex justify-between flex-row md:items-center'>
        ...
        <select onChange={onMobileFilterChange} value={commentFilter.key} className="block md:hidden select select-bordered">
          {COMMENT_FILTER.map(elem => {
            return <option
              key={elem.key}
              value={elem.key}
              onSelect={() => setCommentFilter(elem)}
            >
              {elem.label}
            </option>
          })}
